...
export class StorageUtils {
    ...
    get(key: string, defaultValue?: any): any {
        if(this.storageCache[key]){
            return Promise.resolve(ObjectHelper.deepCopy(this.storageCache[key]));
            ...
            if(this.useStorage){
                this.promiseStorageCache[key] = this.storage.getItem(this.storagePrefix+key).then(value => {
                    try {
                        this.storageCache[key] = JSON.parse(value) || ObjectHelper.deepCopy(defaultValue);
                    } catch(e) {
                        this.storageCache[key] = ObjectHelper.deepCopy(defaultValue);
                    }
                    delete this.promiseStorageCache[key];
                    return ObjectHelper.deepCopy(this.storageCache[key]);
                }, error => {
    ...
    set(key: string, value: any): Promise<void> {
        if(!ObjectHelper.deepEquals(this.storageCache[key], value)){
            this.storageCache[key] = ObjectHelper.deepCopy(value);
            if(this.useStorage){
